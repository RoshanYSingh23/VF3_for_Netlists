...
// map BGL vertex_name → VF3 label
int normalize_gate_type(int t) {
    switch(t) {
...
// minimal graph wrapper (adj + label)
struct SimpleGraph {
    int n;
    ...
    SimpleGraph(const bgl_graph& B) {
        ...
        for(auto v: make_iterator_range(vertices(B)))
            label[idx[v]] = normalize_gate_type(vt[v]);
...
int main(){
    ...
    // **match largest pattern first** (NAND3→NAND2→INV)
    lib.standard_reverse_sort();
