  ...
  class Vf2pp
  {
    private:
        ...
        bool feas(const node n1,const node n2)
        {
            ...
            IncEdgeIt e_test = _g1.edge_iterator(n1);
            for(IncEdgeIt e1 = _g1.edge_iterator(n1); e1.isValid(); ++e1,++e_test)
            {
        ...
        ///The labels must be the numbers {0,1,2,..,K-1}, where K is the number of
        ///different labels.
        Vf2pp(Graph& g1, Graph& g2, std::vector<node>& m, vertex_map intLabels1, vertex_map intLabels2, edge_map intLabelsEdge1, edge_map intLabelsEdge2) :
        _g1(g1), _g2(g2), 
        _depth(0), _mapping(m), _order(g1.node_count(),INVALID),
        _conn(g2,0), _currEdgeIts(g1.node_count(),invalidIncEdgeIt), 
        _rNewLabels1(_g1),_rInOutLabels1(_g1), 
        _intLabels1(intLabels1) ,_intLabels2(intLabels2),
