...
namespace detail
{
    ...
    public:
        ...
        {
            // check if w is a in && out neighbour of a vertex in s
            if (state1_.term_both() && state2_.term_both())
                return state2_.term_both(w);
            // if node was not mapped in "in" yet, then
            // check if w is a out neighbour of a vertex in s
            else if (state1_.term_out() && state2_.term_out())
                return state2_.term_out(w);
            // or if node was not mapped in "out" yet, then
            // check if w is a "in" neighbour of a vertex in s
            else if (state1_.term_in() && state2_.term_in())
                return state2_.term_in(w);
            // otherwise, we have no mapping
            // return if it is not present in mapping
            else
